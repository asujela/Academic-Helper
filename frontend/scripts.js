$(document).ready(function () {
    ...
    var user = {
        ...
        booleancheck: [false, false, false, false, false],
    }
    ...
    function check_boxes(user, booleancheck) {
        ...
        if (user.division.checked == true) {
            user.booleancheck[2] = true;
        }
        else {
            user.booleancheck[2] = false;
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (value in classes) {
                    ...
                    tbody += "Desc";//JSON.stringify(obj.Desc[cname]).replace(/['"]+/g, '') + "\n";
                    tbody += "\nPrerequisites: ";//+JSON.stringify(obj.Preq[cname]).replace(/['"]+/g, '') + "\n";
                    tbody += '</span>';
                    tbody +='</td>';
					
					//Create cell for GE
					tbody += '<td>';
                    tbody += "GE's";//JSON.stringify(obj.GE[cname]).replace(/['"]+/g, '');
                    tbody += '</td>';
                    tbody += '</tr>\n';
                }
                var tfoot = '</table></div>';
